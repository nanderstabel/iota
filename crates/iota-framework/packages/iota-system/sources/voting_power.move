...
public(package) fun set_voting_power(
    ...
    let threshold = TOTAL_VOTING_POWER.min(MAX_VOTING_POWER.max(TOTAL_VOTING_POWER.divide_and_round_up(committee_members.length())));
...
fun init_voting_power_info(
    ...
    validators.take_do_with_ix_ref!(committee_members, |_, validator_index, validator| {
        ...
        let adjusted_stake =
            stake as u128 * (TOTAL_VOTING_POWER as u128) / (total_committee_stake as u128);
...
public(package) fun total_committee_stake(
    validators: &vector<ValidatorV1>,
    committee_members: &vector<u64>,
): u64 {
