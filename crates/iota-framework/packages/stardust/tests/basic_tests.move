...
fun demonstrate_claiming_ptb() {
    ...
    let output = basic_output::create_for_testing(
        ...
        native_tokens,
        option::some(
            storage_deposit_return_unlock_condition::create_for_testing(
                sdruc_return_address,
                sdruc_return_amount,
            ),
        ),
        option::some(timelock_unlock_condition::create_for_testing(timelocked_until)),
        option::some(
            expiration_unlock_condition::create_for_testing(
                owner,
    ...
    extracted_native_tokens =
        utilities::extract_and_send_to<TEST_A>(extracted_native_tokens, migrate_to, &mut ctx);

    // Command 3: extract the asset B and send to the user.
    extracted_native_tokens =
        utilities::extract_and_send_to<TEST_B>(extracted_native_tokens, migrate_to, &mut ctx);
    ...
    assert!(
        iota_coin.value() == (initial_iota_in_output - sdruc_return_amount),
        EIotaBalanceMismatch,
    );
