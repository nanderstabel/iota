...
The Notarization suite is composed of two primary components:
...
The core of the suite is the `Notarization` object, a unified Move `struct` that holds the notarized data and its associated metadata. It is a `key` object, meaning it is owned by an address.
